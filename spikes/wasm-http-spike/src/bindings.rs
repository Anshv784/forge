...
pub trait Guest {
    /// Returns the HTTP status code of a GET request as a diagnostic string,
    /// or an error string. Proves outbound WASI HTTP works inside a
    /// wasm32-wasip2 component under wasmtime's `-S http=y`, the same
    /// mechanism ZeroClaw's own host wires up when a plugin manifest
    /// requests the `http_client` permission.
    fn run() -> Result<_rt::String, _rt::String>;
}
#[doc(hidden)]
macro_rules! __export_world_spike_cabi {
...
#[doc(hidden)]
pub(crate) use __export_world_spike_cabi;
...
#[allow(unused_macros)]
#[doc(hidden)]
macro_rules! __export_spike_impl {
    ($ty:ident) => {
        self::export!($ty with_types_in self);
    };
    ($ty:ident with_types_in $($path_to_types_root:tt)*) => {
        $($path_to_types_root)*:: __export_world_spike_cabi!($ty with_types_in
        $($path_to_types_root)*);
    };
}
#[doc(inline)]
pub(crate) use __export_spike_impl as export;
#[cfg(target_arch = "wasm32")]
#[unsafe(
    link_section = "component-type:wit-bindgen:0.41.0:component:wasm-http-spike:spike:encoded world"
)]
#[doc(hidden)]
#[allow(clippy::octal_escapes)]
pub static __WIT_BINDGEN_COMPONENT_TYPE: [u8; 6468] = *b"\
\0asm\x0d\0\x01\0\0\x19\x16wit-component-encoding\x04\0\x07\xc81\x01A\x02\x01A\x18\
...
ttp/outgoing-handler@0.2.2\x05\x0e\x01j\x01s\x01s\x01@\0\0\x0f\x04\0\x03run\x01\x10\
\x04\0\x1fcomponent:wasm-http-spike/spike\x04\0\x0b\x0b\x01\0\x05spike\x03\0\0\0\
